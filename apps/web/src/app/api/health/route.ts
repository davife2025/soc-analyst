...
export async function GET() {
  ...
  const [
    ...
    db.from('agent_heartbeats').select('*').order('created_at', { ascending: false }).limit(1).maybeSingle(),
    db.from('agent_heartbeats').select('status, created_at, alerts_processed_1h')
      .order('created_at', { ascending: false }).limit(60),
    db.from('alerts').select('*', { count: 'exact', head: true }).eq('status', 'new'),
    db.from('alerts').select('*', { count: 'exact', head: true }),
    db.from('investigations').select('*', { count: 'exact', head: true }),
  ])
  ...
  // Uptime percentage from last 60 heartbeats
  const uptimePct = recent?.length
    ? Math.round((recent.filter(h => h.status === 'healthy').length / recent.length) * 100)
    : 0

  // Throughput over last hour (avg from recent heartbeats)
  const avgProcessed1h = recent?.length
    ? Math.round(recent.reduce((s, h) => s + (h.alerts_processed_1h ?? 0), 0) / recent.length)
    : 0
  ...
  return NextResponse.json({
    ...
    queue: {
      depth: queueDepth ?? 0,
      processed_1h: avgProcessed1h,
    },
    totals: {
      alerts: totalAlerts ?? 0,
      investigations: totalInvs ?? 0,
    },
