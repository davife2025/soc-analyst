...
import { AlertCard, StatusPill } from '@soc/ui'
...
interface Props { initialAlerts: Alert[]; initialInvestigations: Investigation[] }
          ...
          <div className={styles.list}>
            {investigations.map(inv => (
              <div key={inv.id} className={styles.invCard}
                ...
                <div className={styles.invMeta}>{new Date(inv.created_at).toLocaleString()}</div>
              </div>
            ))}
